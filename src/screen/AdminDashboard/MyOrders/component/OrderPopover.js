...
export default function TransitionsModal({ data }) {
  ...
  const {setOrderDetails, orderDetails, setDetailsFlag, detailsFlag} = data
  const BaseUrl = useSelector(BASE_URL)
  const handleClose = () => {
    setDetailsFlag(false);
    setOrderDetails({})
  };
  const { agent, amount, customer, deliverFiles, files, id, orderStatus, revisions, createdAt, updatedAt, serviceType, description } = orderDetails
  console.log("Order Popover",detailsFlag)
  return (
    <div>
      <Modal
        ...
        <Fade in={detailsFlag}>
          <div className={classes.paper}>
            ...
            <div className={classes.innerPaper1}>
              ...
              <h1 className={classes.descText}>RECEIVED AT: {createdAt}</h1>
              <h1 className={classes.descText}>DELIVERED AT: {updatedAt}</h1>
              ...
              <div className={classes.fileContainer}>
                {
                  files.map((v, i) => {
                    ...
                    if (v.slice(-3) === 'png'  || v.slice(-3) === 'PNG' || v.slice(-3) === 'jpg' || v.slice(-3) === 'JPG' || v.slice(-3) === 'jpeg') {
                      return (
